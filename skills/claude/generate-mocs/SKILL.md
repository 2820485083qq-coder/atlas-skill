...
- 如果共享配置文件不存在或 `VAULT_PATH` 为空，停止执行并报告缺失配置。
- 如果 `NOTES_PATH` 或 `CONCEPTS_PATH` 不存在，停止执行，不创建猜测路径。
- 如果任一生成脚本失败，报告失败脚本、退出原因和已完成的前置步骤，不继续 git 自动化。
- 如果脚本运行后没有任何目录页变化，报告“无变更”，不要强行 commit。

## CHECKPOINTS

- STOP：当 `GIT_COMMIT_ENABLED=true` 时，先展示将要提交的目录页变更数量，再 commit。
- STOP：当 `GIT_PUSH_ENABLED=true` 时，先确认远端和分支，再 push。
...
- 不要在路径或配置缺失时猜测 Obsidian vault 位置
- 不要把没有 staged changes 的空提交当作成功
