...
- 如果富化 JSON 不存在或无法解析，停止点评，不编造候选论文。
- 如果 `VAULT_PATH`、`NOTES_PATH` 或 `DAILY_PAPERS_PATH` 不存在，停止保存步骤并报告缺失路径。
- 如果候选池不足 20 篇，按可用数量输出，并在末尾说明候选耗尽。
- 如果写入推荐文件失败，保留已生成的点评文本并报告目标路径。

## CHECKPOINTS

- STOP：当 `GIT_COMMIT_ENABLED=true` 时，先展示将要提交的推荐文件和 history 文件，再 commit。
- STOP：当富化数据缺少关键字段（标题、链接、摘要）超过 5 篇时，先报告数据质量问题。
