...
- 如果富化 JSON 无法解析，停止执行，报告文件路径和解析错误，不生成笔记。
- 如果推荐文件中没有“必读”论文，跳过笔记生成并明确说明没有目标论文。
- 如果 Task agent 调用 `/paper-reader` 失败，记录失败论文标题和链接，继续处理下一篇；最后汇总失败列表。
- 如果回填链接失败，保留已生成笔记，报告推荐文件路径和未回填论文，不重新生成已合格笔记。

## CHECKPOINTS

- STOP：当“必读”论文超过 5 篇时，先说明预计耗时和数量，再继续批量生成。
- STOP：当质量验证连续失败 2 次时，停止该论文重试，报告原因并继续下一篇。
...
- 不要在富化数据、推荐文件或 paper-reader 输出缺失时编造论文笔记
