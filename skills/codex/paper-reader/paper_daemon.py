...
def _resolve_claude_command() -> str:
    """Resolve Claude CLI command on different platforms."""
...
def wait_for_quota_reset(wait_seconds: Optional[int] = None):
    ...
    logger.info(f"[WAIT] 配额受限，等待 {wait_minutes} 分钟...")
...
def copy_zotero_db() -> str:
    """复制 Zotero 数据库以避免锁定"""
    state_dir = Path(_DAEMON_STATE_DIR)
    state_dir.mkdir(parents=True, exist_ok=True)
    tmp_db = state_dir / "zotero_readonly.sqlite"
...
请直接开始处理，不需要确认。提取所有公式、图片和表格。"""
    ...
    try:
        result = subprocess.run(
            [CLAUDE_COMMAND, '-p', prompt, '--model', 'opus', '--permission-mode', 'acceptEdits', '--dangerously-skip-permissions'],
            capture_output=True,
            text=True,
            encoding='utf-8',
            errors='replace',
...
def process_collection(collection_name: str, resume: bool = True):
    ...
    for paper in papers:
        ...
        title = paper['title']
        title_key = _normalize_method_name(title or str(item_id))

        if item_id in progress['completed']:
            continue

        if title_key in seen_titles:
            logger.info(f"跳过重复条目: {title[:50]}")
    ...
    for i, paper in enumerate(pending):
        ...
        if success:
            ...
            logger.warning(f"[WAIT] Rate limit, 等待 {wait_time} 秒...")
            ...
            if reset_wait:
                logger.warning(f"[WAIT] 用量上限，等待到重置（约 {reset_wait // 60} 分钟）...")
            ...
            logger.error(f"[FAIL] 超时: {title[:50]}")
            progress['failed'].append({'item_id': item_id, 'title': title, 'error': 'TIMEOUT'})
            save_progress(progress)

        else:
            logger.error(f"[FAIL] 失败: {title[:50]} - {error[:100]}")
