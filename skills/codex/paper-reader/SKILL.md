...
- 如果输入路径不存在，先要求用户给出有效 PDF、URL 或 Zotero 条目，不生成示例笔记。
- 如果 Zotero 条目找不到 PDF 且无法找到 arXiv、DOI 或网页全文，明确标记为无法读取，不编造论文内容。
- 如果 arXiv HTML、PDF、项目主页图片都失败，仍可生成文字笔记，但必须标记图片缺失原因。
- 如果公式、Figure、Table 提取不完整，报告缺失项并优先补抓，不把骨架笔记当作完成。

## CHECKPOINTS

- STOP：当批量读取论文超过 5 篇时，先列出候选论文和预计输出位置，再继续。
- STOP：当需要写入 Obsidian vault、刷新 MOC 或执行 git commit/push 时，先确认配置变量和目标路径。

## 不要做

- 不要在没有论文全文、摘要或 PDF 的情况下输出“完整解析”。
- 不要把快速摘要伪装成完整笔记。
- 不要跳过 Figure、公式、表格清单验证。
- 不要为了赶进度自己手写简化版笔记代替模板输出。
