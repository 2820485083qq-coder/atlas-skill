...
- If `OPENAI_API_KEY` is missing, stop before live calls and tell the user how to set it locally.
- If an input image or mask is missing, stop and ask for the correct path instead of switching to generation.
- If the API fails or returns no image, report the error and preserve the prompt/spec for retry.
- If text rendering is wrong, iterate once with verbatim text constraints before changing the concept.

## CHECKPOINTS

- STOP before spending API calls on large batches; show the batch count and output directory first.
- STOP before editing a user-supplied image in a way that changes identity, product claims, logos, or exact text.
...
- Do not claim an image was generated or edited unless an output file exists.
- Do not overwrite input images.
