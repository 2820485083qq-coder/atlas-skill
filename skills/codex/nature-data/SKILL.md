...
- If the target journal or article type is missing, use Nature Portfolio defaults and mark the assumption.
- If repository identifiers, accession numbers, licences, or embargo details are unknown, leave them as missing fields; do not invent them.
- If data are restricted, require the restriction reason, controller, request route, and review criteria before final wording.
- If policy details are time-sensitive, verify the current journal page before final submission wording.

## CHECKPOINTS

- STOP before asserting that data are publicly available without a repository link or stable identifier.
- STOP before recommending “available upon request” unless a concrete restriction and access process exist.

## Do Not Do

- Do not fabricate DOIs, accessions, ethics approvals, licences, or repository names.
- Do not turn a Data Availability statement into a methods rewrite.
- Do not hide missing data behind vague wording.
