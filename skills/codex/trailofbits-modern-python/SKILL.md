...
- If `uv`, `ruff`, `ty`, or `prek` is unavailable, report the missing tool and use the smallest install/setup step rather than rewriting project config blindly.
- If the project already has binding legacy tooling and the user did not ask to migrate, preserve it and only add compatible improvements.
- If dependency migration fails for a package, keep the original dependency entry visible and mark it for manual review.
- If tests, linting, or type checks fail after setup, report the failing command and do not claim the migration is complete.

## CHECKPOINTS

- Before editing config: identify whether the task is a script, app, package, or migration.
- Before deleting legacy files: confirm the new `pyproject.toml` and lockfile can install successfully.
- Before final answer: run the relevant `uv run` validation commands or state exactly why they could not be run.
- Final output should include the commands the user can rerun, not just a description of the tooling.
