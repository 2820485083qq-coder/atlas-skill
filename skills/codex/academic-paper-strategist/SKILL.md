...
- If the repository root, draft, or school sample path is missing, pause planning and request the smallest missing input instead of inventing structure.
- If evidence files contradict the draft, prefer repository evidence and mark the draft claim for deletion or downgrade.
- If a detection report is image-only or unreadable, ask for the source PDF/text or classify only the visible pages without extrapolating.
- If the task requires final DOCX delivery, hand off to the composer/doc flow rather than claiming the strategist output is final.
- If runtime screenshots are required but the app cannot be launched, produce a blocked screenshot plan with exact missing prerequisites.

## CHECKPOINTS

- Before outlining: identify the binding school/template constraints and deliverable path.
- Before allowing any claim: map it to a concrete code, document, schema, runtime, or supplied draft source.
- Before handoff: list preserved assets, replacement boundaries, unsupported claims, and risky manual checks.
- Final output must state that it is a planning/handoff artifact, not a submission-ready thesis.
