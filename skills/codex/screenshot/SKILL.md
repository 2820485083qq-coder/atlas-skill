...
- If the requested app/window is not found, list likely open windows or ask for the exact app/window name.
- If OS permissions block capture, run the preflight helper where available and report the permission needed.
- If a region capture is requested with invalid coordinates, ask for corrected `x,y,w,h` values.
- If multiple windows or displays are captured, inspect or report each output path instead of assuming one image.

## CHECKPOINTS

- STOP before capturing sensitive full-screen content when a narrower app/window/region would satisfy the request.
- STOP before saving outside the user-specified path or temp/default screenshot location.

## Do Not Do

- Do not use OS-level screenshots when a more precise tool-specific capture is available and suitable.
- Do not manipulate or enhance screenshots unless the user asked.
- Do not claim visual findings without viewing the captured image.
