...
- If the Figma MCP server is unavailable, stop implementation and report the setup or login step that failed.
- If the provided URL has no usable node ID, ask for a frame/component link or a selected desktop node before coding.
- If `get_design_context` is truncated, switch to metadata-first decomposition and fetch child nodes instead of guessing missing layout.
- If assets cannot be downloaded or the `localhost` asset URL is unavailable, document the missing asset and block final parity claims.
- If project conventions conflict with the raw Figma output, preserve the project convention and explicitly list the visual deviation.

## CHECKPOINTS

- Before editing code: design context and screenshot have both been fetched.
- Before creating components: existing project components and design tokens have been checked.
- Before final answer: UI has been compared against the Figma screenshot for layout, typography, colors, assets, responsiveness, and accessibility.
- Final answer must name any unresolved parity gaps instead of claiming pixel-perfect completion.

## Do Not Do

- Do not implement from a Figma link using visual memory or assumptions alone.
- Do not replace real Figma assets with placeholders when an asset endpoint is available.
- Do not add new icon/UI packages before checking existing project dependencies.
- Do not claim 1:1 parity unless a screenshot comparison was actually performed.
