...
- Before running a review: scope, tool choice, project context inclusion, and focus are known.
- Before invoking external tools: show or inspect diff stats and stop on empty diffs.
- Before security scanning: confirm dependency manifests changed; otherwise skip dependency scanning.
- Before final output: present findings by severity with file/line references where the external review provides them.

## Do Not Do

- Do not run Gemini with broad review prompts on secrets or private diffs unless the user selected it.
- Do not run dependency scans when no dependency files changed.
- Do not present external model output as authoritative; summarize agreement, disagreement, and residual uncertainty.
- Do not claim a clean review if a selected tool failed or timed out.
