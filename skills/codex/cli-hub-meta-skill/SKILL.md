...
- If `cli-hub` is not found, locate `cli-hub.exe` in the Python user scripts directory or install it with `pip install cli-anything-hub`.
- If install fails, run `cli-hub info <name>` first and report the required backend, package source, and install command.
- If a CLI requires a desktop app, API key, or local service, verify that dependency before launching the CLI.
- If the catalog is unavailable, use `cli-hub list` from the local registry cache if present; otherwise report that discovery is blocked.

## CHECKPOINTS

- STOP before installing a CLI that controls real software, external accounts, paid APIs, or local files; state what it will install and wait for user confirmation.
- STOP before running a command that writes, deletes, publishes, sends, uploads, or changes external state.

## Do Not Do

- Do not install a CLI just because it appears in search results; match it to the user's concrete target app or workflow.
- Do not run state-changing commands without confirmation.
- Do not treat CLI-Hub as an MCP server. It is a package manager and launcher for CLI tools.
