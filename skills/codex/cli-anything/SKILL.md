...
- If cloning or downloading the target repository fails, stop and report the network or authentication failure; do not invent a harness from README snippets alone.
- If the target software has no callable backend, executable, scripting API, or documented automation surface, mark the harness as blocked and list the missing interface.
- If a real backend exists but is not installed locally, create only the harness scaffold and tests that can run without the backend; label real E2E validation as pending.
- If generated commands cannot round-trip state or JSON output, fix those before adding more commands.

## CHECKPOINTS

- STOP after backend discovery: confirm whether the harness wraps the real backend, a documented API, or a synthetic fallback.
- STOP before removing or renaming existing commands in refine mode.
...
- Do not build a fake CLI that only simulates the software when a real backend is available.
- Do not skip `--json` support, REPL startup, or installable entry points.
- Do not report the harness as complete until local installation and at least one command path have been verified.
